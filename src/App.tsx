...
import EditorPane, { DEFAULT_CODE } from "./components/EditorPane";
...
function reducer(state: AppState, action: UIAction): AppState {
  switch (action.type) {
...
function initialState(): AppState {
  ...
  return {
    code: DEFAULT_CODE,                // ✅ 修复：code state 不再为空
    chatCollapsed: false,
...
export default function App() {
  ...
  // Snapshot Throttling (2.7-C)
  useEffect(() => {
    const timer = setTimeout(() => {
       if (state.code && state.code !== DEFAULT_CODE) {
          const cursor = editorApiRef.current?.getCursor() || undefined;
          const sel = editorApiRef.current?.getSelectionRange() || undefined;
          api.saveCodeSnapshot(state.code, cursor, sel, "main.py").catch(e => console.error("Snapshot failed", e));
       }
    }, 2000); // 2s debounce
  ...
  async function runProgram() {
    ...
    try {
      const cs = await api.createCodeState(state.code, traceId);
      codeStateId = cs.code_state_id;
    } catch {}

    const cursor = editorApiRef.current?.getCursor() || undefined;
    const sel = editorApiRef.current?.getSelectionRange() || undefined;
    await api.saveCodeSnapshot(state.code, cursor, sel, "main.py").catch(() => {});

    api.reportEvent("run", { run_id: uid("run"), run_command: "python", cwd: ".", code_len: state.code.length }, traceId, codeStateId).catch(() => {});
    try {
      const res = await api.runCode(state.code);
  ...
  async function runTests() {
    ...
    api.reportEvent("test", { test_id: uid("test"), run_command: "python -m pytest", cwd: ".", code_len: state.code.length }, traceId, codeStateId).catch(() => {});
    try {
      const res = await api.testCode(state.code);
      if (res.stdout) dispatch({ type: "CONSOLE_APPEND", entries: [{ id: uid("out"), kind: "log", text: res.stdout.trimEnd() }] });
      if (res.stderr) dispatch({ type: "CONSOLE_APPEND", entries: [{ id: uid("err"), kind: "error", text: res.stderr.trimEnd() }] });
      if (res.timed_out) dispatch({ type: "CONSOLE_APPEND", entries: [{ id: uid("to"), kind: "error", text: "Timed out" }] });
  ...
  async function saveNow() {
    ...
    const sel = editorApiRef.current?.getSelectionRange() || undefined;
    await api.saveCodeSnapshot(state.code, cursor, sel, "main.py").catch(() => {});
    api.reportEvent("edit", { event_subtype: "save", file_path: "main.py", cursor_line: cursor?.line, cursor_col: cursor?.col }).catch(() => {});
      ...
      <div className="bodyRow">
        ...
        <div className="editorAndConsole">
          <EditorPane
            ...
            code={state.code}
            onCodeChange={(c) => dispatch({ type: "SET_CODE", code: c })}
